...
// ➔ ВАЖНО: Сначала начисляем очки, ПОКА selLetter не обнулился
  if (window.currentUserId && window.firestoreUpdateStats) {
      ...
      window.firestoreUpdateStats(
          ...
          ptsForLetter,
          null // <-- null гарантирует, что "слово" не создастся и Walks не вырастет
      ).catch(e => console.error('Ошибка начисления:', e));
  }

  // Если была жалоба... (код жалоб оставляем как есть)
  if (window._reportEditing && window._reportEditing.reason) {
     // ...
  }

  // ➔ ТОЛЬКО ПОТОМ сбрасываем форму
  resetUploadForm();
